...
async function searchPapers(query, { mode = "standard", limit = 10 } = {}) {
  ...
  try {
    ...
    const input = page.locator(
      'input[type="search"], input[placeholder*="earch"], input[placeholder*="question"], textarea'
    );
    ...
    const submit = page.locator(
      'button[type="submit"], button[aria-label*="earch"], button:has(svg)'
    );
    ...
    // Wait for either results or navigation
    ...
    // Extract paper links and surrounding text
    const papers = await page.evaluate(() => {
      ...
      for (const a of anchors) {
        ...
        seen.add(href);
        const title = (a.innerText || "").trim().slice(0, 300);
        if (!title) continue;
